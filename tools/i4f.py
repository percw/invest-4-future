...
playbook can be corrected on evidence. board turns recorded prices into
buy / sell signals; journal is the channel for messages back.
"""
import argparse
import calendar
import json
...
def _f(value):
    """Parse a number from a table cell, or None (handles '—', '', thousands)."""
    try:
...
def _n(value):
    """Format a number for a table cell: drop a trailing .0, None becomes '-'."""
    if value is None:
        return "-"
...
_ZONE_RE = re.compile(r"(\d+(?:\.\d+)?)\s*-\s*(\d+(?:\.\d+)?)\s*([A-Za-z]{3})")


def parse_zone(text):
    """Parse an entry_zone like '260-285 USD' into (low, high, ccy), or None."""
    m = _ZONE_RE.search(str(text))
    return (float(m.group(1)), float(m.group(2)), m.group(3).upper()) if m else None


def parse_table(text):
    """Parse the first GitHub-style markdown table into a list of dict rows."""
    rows, header = [], None
    for line in text.split("\n"):
        ...
        if header is None:
            header = cells
        elif "".join(cells).strip() and set("".join(cells).replace(" ", "")) <= {"-", ":"}:
            continue
        else:
...
# --------------------------------------------------------------------------
# board + journal - the decision / communication layer
# --------------------------------------------------------------------------
def _board_text(recs, hyps, prices, positions):
    asofs = sorted({a for _, _, a in prices.values() if a and a not in ("-", "—")})
    asof = ", ".join(asofs) if asofs else "no prices recorded - refresh portfolio/prices.md"
    ...
    buys = sorted(
        (r for r in recs if r[1].get("status") in ("NEW", "WATCHING")),
        key=lambda r: r[1].get("ticker", ""),
    )
    buy_n = 0
    for _, fm, _ in buys:
        ticker = fm.get("ticker", "")
        zone = parse_zone(fm.get("entry_zone", ""))
        pr = prices.get(ticker)
        price = pr[0] if pr else None
        hstatus = hyps.get(fm.get("hypothesis_id"), {}).get("status")
        if hstatus and hstatus != "ACTIVE":
            signal = f"SKIP - thesis {hstatus}"
        elif price is None:
            signal = "NO PRICE - refresh prices.md"
        elif zone is None:
            signal = "? - entry_zone unparseable"
        elif price < zone[0]:
            signal, buy_n = "BUY - below entry zone", buy_n + 1
        elif price <= zone[1]:
            signal, buy_n = "BUY - in entry zone", buy_n + 1
        else:
            signal = "WAIT - above entry zone"
        price_cell = f"{_n(price)} {pr[1]}" if price is not None else "-"
        out.append(
            f"| {ticker} | {signal} | {price_cell} | {fm.get('entry_zone', '')} | "
            f"{fm.get('conviction', '')} | {_n(fm.get('position_size_pct'))} % |"
        )
    if not buys:
        out.append("| _none_ |  |  |  |  |  |")
    ...
    for row in positions:
        ...
        if not ticker:
            continue
        qty, gav = _f(row.get("Qty")), _f(row.get("GAV"))
        ccy = row.get("Ccy", "").strip()
        hypid = row.get("Hyp-ID", "").strip()
        hstatus = hyps.get(hypid, {}).get("status")
        if not hypid or hypid in ("-", "—"):
            note = "hold - no linked thesis"
        elif hstatus == "FALSIFIED":
            note = "SELL - thesis falsified"
            sell_rows.append((ticker, "SELL", f"linked hypothesis {hypid} is FALSIFIED"))
        elif hstatus == "EXPIRED":
            note = "review - thesis expired"
            sell_rows.append((ticker, "REVIEW", f"linked hypothesis {hypid} is EXPIRED"))
        elif hstatus == "REJECTED":
            note = "review - thesis rejected"
            sell_rows.append((ticker, "REVIEW", f"linked hypothesis {hypid} is REJECTED"))
        else:
            note = "hold - thesis intact"
        price = (prices.get(ticker) or (None,))[0]
        if price is not None and qty is not None and gav not in (None, 0):
            value = qty * price
            out_pl = f"{(price / gav - 1) * 100:+.1f} %"
            perf_rows.append(
                (ticker, _n(qty), f"{_n(gav)} {ccy}", f"{_n(price)} {ccy}",
                 f"{_n(round(value, 2))} {ccy}", out_pl, note)
            )
        else:
            perf_rows.append(
                (ticker, _n(qty), f"{_n(gav)} {ccy}" if gav is not None else "-",
                 "-", "-", "-", note)
            )

    out += ["", "## Sell / review signals", ""]
    if sell_rows:
        out += ["| Ticker | Signal | Reason |", "|---|---|---|"]
        out += [f"| {t} | {s} | {r} |" for t, s, r in sell_rows]
    else:
        out.append("None - every held thesis is still ACTIVE.")
    ...
    if perf_rows:
        out += ["| " + " | ".join(str(c) for c in r) + " |" for r in perf_rows]
    else:
...
def cmd_board(args):
    recs = load_dir("recommendations")
    hyps = {fm.get("id"): fm for _, fm, _ in load_dir("hypotheses")}

    prices = {}
    prices_path = ROOT / "portfolio" / "prices.md"
    if prices_path.exists():
        for row in parse_table(prices_path.read_text(encoding="utf-8")):
            ticker = row.get("Ticker", "").strip()
            if ticker:
                prices[ticker] = (
                    _f(row.get("Price")),
    ...
    positions = []
    positions_path = ROOT / "portfolio" / "positions.md"
    if positions_path.exists():
        positions = parse_table(positions_path.read_text(encoding="utf-8"))

    text, buy_n, sell_n = _board_text(recs, hyps, prices, positions)
    (ROOT / "portfolio" / "action-board.md").write_text(text, encoding="utf-8")
    print(
        f"rewrote portfolio/action-board.md - {buy_n} buy signal(s), "
        f"{sell_n} sell/review signal(s)"
    )
    return 0
...
def cmd_journal(args):
    ...
    if not message:
        print("ERROR: empty journal message", file=sys.stderr)
        return 1
    path = ROOT / "portfolio" / "journal.md"
    existing = path.read_text(encoding="utf-8") if path.exists() else "# Journal\n\n## Entries\n"
    path.write_text(
        existing.rstrip("\n") + f"\n- {date.today().isoformat()}: {message}\n",
        encoding="utf-8",
    )
    print(f"journaled ({date.today().isoformat()}): {message}")
    return 0
...
def main(argv=None):
    ...
    sub.add_parser(
        "board", help="rebuild portfolio/action-board.md (buy/sell/performance)"
    ).set_defaults(func=cmd_board)

    p_journal = sub.add_parser("journal", help="append a message to portfolio/journal.md")
    p_journal.add_argument("message", nargs="+", help="message text")
